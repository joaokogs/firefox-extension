...
This repository contains the editable source code for Prismi Dashboard v16.1.2. It does not include vendored third-party libraries or production build output.
...
This creates the unpacked package in `dist-chrome/` and the upload-ready ZIP at `release/prismi-dashboard-v16.1.2-chrome.zip`. The ZIP has `manifest.json` at its root and removes Firefox-only metadata. The Chrome build requires `VITE_GOOGLE_CLIENT_ID` in `.env.local`; the `.env.local` file is not included in the ZIP.
